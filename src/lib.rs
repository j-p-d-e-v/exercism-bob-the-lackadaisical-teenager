pub fn reply(message: &str) -> &str {
    ...
    let has_letters: bool = message.to_lowercase().replace(&['?','!'],"").chars().any(|c| matches!(c,'a'..='z'));
    let is_uppercase: bool = message == message.to_uppercase();
    if message.is_empty() {
        "Fine. Be that way!"
    }
    else if message.ends_with('?'){
        if is_uppercase && has_letters{
    ...
    else if is_uppercase && has_letters {
        "Whoa, chill out!"
    }
    else {
        "Whatever."
